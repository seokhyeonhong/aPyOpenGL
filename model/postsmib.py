...
class PositionContextTransformer(nn.Module):
    def __init__(self, noj, num_layers=6, num_heads=8, d_model=512, d_ff=2048):
        super(PositionContextTransformer, self).__init__()
        ...
        self.decoder = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.PReLU(),
            nn.Linear(d_model, noj*6 + 3),
        )

    def forward(self, x, batch_mask, masked_joint, joint_mask, p_kf):
        """
        :param x: (B, T, D)
        :param batch_mask: (B, T, D)
        :param p_kf: (B, T, 2)
        """
        B, T, D = x.shape
        device = x.device
        ...
        h_ctx = self.encoder(torch.cat([x, batch_mask, masked_joint, joint_mask], dim=-1)) + pe_kf # (B, T, d_model)
        ...
        m_in = torch.sum(batch_mask, dim=-1) # (B, T)
