...
class MotionApp(agl.AnimApp):
    ...
    def start(self):
        ...
        self.ui.add_menu_item("MotionApp", "X-Ray", self.switch_xray, key=glfw.KEY_X)
        self.ui.add_menu_item("MotionApp", "Model", self.render_model.switch_visible, key=glfw.KEY_M)

    def switch_xray(self):
        self.render_skeleton = not self.render_skeleton
    ...
    def update(self):
        ...
        if self.focus_on_root:
            self.camera.set_focus_position(self.motion.poses[self.curr_frame].root_pos)
            self.camera.set_up(glm.vec3(0, 1, 0))
        elif self.follow_root:
            self.camera.set_position(self.motion.poses[self.curr_frame].root_pos + glm.vec3(0, 1.5, 5))
            self.camera.set_focus_position(self.motion.poses[self.curr_frame].root_pos)
            self.camera.set_up(glm.vec3(0, 1, 0))
        self.camera.update()
    ...
    def render_xray(self):
        ...
        if self.render_skeleton:
            agl.Render.skeleton(self.model).draw()
