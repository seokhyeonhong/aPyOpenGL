...
class Joint:
    ...
    def __init__(
        ...
        self.local_pos = np.array([0, 0, 0], dtype=np.float32) if local_pos is None else np.array(local_pos, dtype=np.float32)
        ...
        if self.local_pos.shape != (3,):
            raise ValueError(f"Local position must be a 3-dimensional vector, but got {self.local_pos.shape}.")

    def pre_xform(self):
        pre_rotmat = n_quat.to_rotmat(self.pre_quat)
        return n_rotmat.to_xform(pre_rotmat, translation=self.local_pos)