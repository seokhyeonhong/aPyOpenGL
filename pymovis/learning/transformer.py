...
class CrossRelMultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_head, n_head, dropout=0.1, pre_layernorm=True):
        super(CrossRelMultiHeadAttention, self).__init__()
        self.d_model = d_model
        self.d_head = d_head
        self.n_head = n_head
        self.pre_layernorm = pre_layernorm
    ...
    def forward(self, x, context, lookup_table, mask=None):
        ...
        # cross attention score (B, n_head, T, S)
        QE_t = torch.matmul(q, lookup_table.transpose(-2, -1)) * self.atten_scale
        QE_t = self.skew(QE_t)
        atten_score = atten_score + QE_t
